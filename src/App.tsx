...
import {
  ...
  fetchUserCourseRequirementsForUser, saveCourseFeedback,
} from './lib/data';
...
import { EmployeeNotifications, EmployeeGamification, EmployeeCertifications, EmployeeProfile, CourseFeedbackModal, type EmployeeStrings } from '@/components/employee/EmployeeModules';
...
const copy = {
  ES: {
    ...
    notifications: 'Notificaciones', noNotifications: 'No hay notificaciones', markAllRead: 'Marcar todas como leídas', gamification: 'Gamificación', myProfile: 'Mi perfil', roleCertification: 'Certificación de cargo', noCertificates: 'No tienes certificados aún', noRoleCerts: 'No tienes certificaciones de cargo', noBadges: 'Aún no has ganado insignias', pointsAccumulated: 'Puntos acumulados', badgesEarned: 'Insignias obtenidas', nextBadges: 'Próximas insignias', personalInfo: 'Información personal', jobHistory: 'Historial de cargos', editProfile: 'Editar perfil', saveProfile: 'Guardar perfil', hireDate: 'Fecha de ingreso', currentRoleSince: 'En cargo desde', changePassword: 'Cambiar contraseña', newPassword: 'Nueva contraseña', confirmPassword: 'Confirmar contraseña', updatePassword: 'Actualizar contraseña', passwordChanged: 'Contraseña actualizada correctamente', passwordMismatch: 'Las contraseñas no coinciden', passwordTooShort: 'La contraseña debe tener al menos 6 caracteres', stats: 'Estadísticas', coursesCompleted: 'Cursos completados', coursesInProgress: 'Cursos en progreso', totalPoints: 'Puntos totales', certifiedRole: 'Cargo certificado', certifiedOn: 'Certificado el', expiresOn: 'Expira el', valid: 'Vigente', expired: 'Expirado', noJobHistory: 'No hay historial de cargos', startDate: 'Fecha de inicio', endDate: 'Fecha de fin', reason: 'Razón', currentRole: 'Cargo actual', verifyCert: 'Verificar', certNumber: 'Número de certificado', issuedOn: 'Emitido el', viewBadge: 'Ver insignia', locked: 'Bloqueada', recentActivity: 'Actividad reciente', unread: 'sin leer', profileUpdated: 'Perfil actualizado correctamente', errorUpdating: 'Error al actualizar el perfil', allBadges: 'Todas las insignias', earnedBadges: 'Insignias obtenidas', availableBadges: 'Insignias disponibles', badgePoints: 'Puntos de insignia', badgeCategory: 'Categoría de insignia', earnedOn: 'Obtenida el',
  },
  EN: {
    ...
    notifications: 'Notifications', noNotifications: 'No notifications', markAllRead: 'Mark all as read', gamification: 'Gamification', myProfile: 'My profile', roleCertification: 'Role certification', noCertificates: 'No certificates yet', noRoleCerts: 'No role certifications', noBadges: 'No badges earned yet', pointsAccumulated: 'Points accumulated', badgesEarned: 'Badges earned', nextBadges: 'Next badges', personalInfo: 'Personal information', jobHistory: 'Job history', editProfile: 'Edit profile', saveProfile: 'Save profile', hireDate: 'Hire date', currentRoleSince: 'Current role since', changePassword: 'Change password', newPassword: 'New password', confirmPassword: 'Confirm password', updatePassword: 'Update password', passwordChanged: 'Password changed successfully', passwordMismatch: 'Passwords do not match', passwordTooShort: 'Password must be at least 6 characters', stats: 'Statistics', coursesCompleted: 'Courses completed', coursesInProgress: 'Courses in progress', totalPoints: 'Total points', certifiedRole: 'Certified role', certifiedOn: 'Certified on', expiresOn: 'Expires on', valid: 'Valid', expired: 'Expired', noJobHistory: 'No job history', startDate: 'Start date', endDate: 'End date', reason: 'Reason', currentRole: 'Current role', verifyCert: 'Verify', certNumber: 'Certificate number', issuedOn: 'Issued on', viewBadge: 'View badge', locked: 'Locked', recentActivity: 'Recent activity', unread: 'unread', profileUpdated: 'Profile updated successfully', errorUpdating: 'Error updating profile', allBadges: 'All badges', earnedBadges: 'Earned badges', availableBadges: 'Available badges', badgePoints: 'Badge points', badgeCategory: 'Badge category', earnedOn: 'Earned on',
  },
};
...
function EmployeeApp({ profile, language, setLanguage, dark, setDark }: { profile: Profile; language: Language; setLanguage: (l: Language) => void; dark: boolean; setDark: (d: boolean) => void }) {
  ...
  const [feedbackCourse, setFeedbackCourse] = useState<CourseWithRelations | null>(null);
  ...
  const breadcrumbLabel = () => ({ dashboard: t.dashboard, catalog: t.catalog, courses: t.catalog, certificates: t.certificates, admin: t.adminPanel, calendar: t.calendar, player: t.player, exam: t.exam, notifications: t.notifications, gamification: t.gamification, profile: t.myProfile, 'course-editor': '', team: '', assignments: '', personnel: '', roles: '', 'role-detail': '' })[view] ?? '';

  const completedCourses = courses.filter((c) => getCourseState(c.id).courseExamPassed);
  const inProgressCourses = courses.filter((c) => { const s = getCourseState(c.id); return s.completedModules.some(Boolean) && !s.courseExamPassed; });

  const empStrings: EmployeeStrings = {
    notifications: t.notifications, noNotifications: t.noNotifications, markAllRead: t.markAllRead,
    gamification: t.gamification, myProfile: t.myProfile, certifications: t.certificates, roleCertification: t.roleCertification,
    noCertificates: t.noCertificates, noRoleCerts: t.noRoleCerts, noBadges: t.noBadges,
    pointsAccumulated: t.pointsAccumulated, badgesEarned: t.badgesEarned, nextBadges: t.nextBadges,
    personalInfo: t.personalInfo, jobHistory: t.jobHistory, editProfile: t.editProfile, saveProfile: t.saveProfile,
    fullName: t.fullName, email: t.email, jobRole: t.jobRole, hireDate: t.hireDate, currentRoleSince: t.currentRoleSince,
    changePassword: t.changePassword, newPassword: t.newPassword, confirmPassword: t.confirmPassword,
    updatePassword: t.updatePassword, passwordChanged: t.passwordChanged, passwordMismatch: t.passwordMismatch,
    passwordTooShort: t.passwordTooShort, stats: t.stats, coursesCompleted: t.coursesCompleted,
    coursesInProgress: t.coursesInProgress, totalPoints: t.totalPoints, certifiedRole: t.certifiedRole,
    certifiedOn: t.certifiedOn, expiresOn: t.expiresOn, valid: t.valid, expired: t.expired,
    noJobHistory: t.noJobHistory, startDate: t.startDate, endDate: t.endDate, reason: t.reason,
    currentRole: t.currentRole, downloadCert: t.downloadCert, verifyCert: t.verifyCert,
    certNumber: t.certNumber, issuedOn: t.issuedOn, courseLabel: t.courseLabel, viewBadge: t.viewBadge,
    locked: t.locked, recentActivity: t.recentActivity, viewAll: t.viewAll, unread: t.unread,
    loading: t.loading, profileUpdated: t.profileUpdated, errorUpdating: t.errorUpdating,
    allBadges: t.allBadges, earnedBadges: t.earnedBadges, availableBadges: t.availableBadges,
    badgePoints: t.badgePoints, badgeCategory: t.badgeCategory, earnedOn: t.earnedOn,
  ...
  return <div className={dark ? 'app-shell dark' : 'app-shell'}>
    {!isFullscreen && <aside className={mobileOpen ? 'sidebar open' : 'sidebar'}>
      ...
      <nav>
        ...
        <NavItem icon={<Bell size={19} />} label={t.notifications} active={view === 'notifications'} onClick={() => navigate('notifications')} />
        <NavItem icon={<Trophy size={19} />} label={t.gamification} active={view === 'gamification'} onClick={() => navigate('gamification')} />
        <NavItem icon={<User size={19} />} label={t.myProfile} active={view === 'profile'} onClick={() => navigate('profile')} />
      </nav>
